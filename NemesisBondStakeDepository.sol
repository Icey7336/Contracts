...
contract NemesisBondStakeDepository is Ownable {
    ...
    address public immutable NMS; // intermediate reward token from treasury
    address public immutable sNMS; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sNMS gons remaining to be paid
    ...
    function deposit( 
        ...
        IERC20( NMS ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        IStaking( staking ).claim( address(this) );
        uint stakeGons=IsNMS(sNMS).gonsForBalance(payout);
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = IsNMS(sNMS).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sNMS ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sNMS balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=IsNMS(sNMS).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = IsNMS(sNMS).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
